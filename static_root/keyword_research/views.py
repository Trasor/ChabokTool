...
def keyword_research(request):
    if request.method == 'POST':
        ...
        # پیغام موفقیت
        messages.success(request, f'درخواست "{req.name}" با موفقیت ایجاد و SERP تکمیل شد!')
...
def request_detail(request, pk):
    ...
    # فرآیند مقایسه و تشخیص PKW/AKW (فقط یکبار اجرا میشه)
    all_keywords = Keyword.objects.filter(user=request.user).order_by('id')
    _process_pkw_akw_comparison(all_keywords)
    
    # فقط PKW ها رو به template بفرست (برای نمایش در جدول)
    ...
    return render(request, 'keyword_research/request_detail.html', {
        'req': req,
        'keywords': pkw_keywords  # فقط PKW ها
    })
